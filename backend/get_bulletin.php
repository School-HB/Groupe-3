...
$type = $_GET['type'] ?? 'semestre'; // 'semestre' ou 'annuel'
...
try {
    if ($type === 'annuel') {
        // Logique Annuelle : (Moy S1 + Moy S2) / 2 au niveau des matières
        // ET (Moy Générale S1 + Moy Générale S2) / 2 au niveau global
        ...
        $matiereData = [];
        $semestreTotals = []; // [semestre_id => [total_pondere, total_coeffs]]
        ...
        foreach ($rows as $r) {
            ...
            // Pour la moyenne annuelle de la matière
            ...
            // Pour la moyenne générale du semestre
        ...
        // 1. Calcul des moyennes annuelles matières
        $notes = [];
        ...
        // 2. Calcul du Total Annuel (Global S1 + Global S2) / 2
        $moyennesSemestres = [];
